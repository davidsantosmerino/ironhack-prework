...
function dniLetter( dni ) {
	...
	if(dni > 0 && typeof dni === 'number' ) {
		...
		var lockupArray = lockup.split('');
		result = lockupArray[dni % 23];
	}
	else {
...
function findLongestWord( array_words ){
	var length = array_words.length;
	var maxLength = 0;
	for(var i = 0; i < length; i++){ 
		maxLength = array_words[i].length > maxLength ? array_words[i].length : maxLength;
	}
...
array_lengths.push( findLongestWord( ["Reddasdasd", "Bdasdasdlue", "Gredsadadssssssen"] ) );
...
function charFreq( string ) {
	...
	for(var i = 0; i < length; i++){
		if(!freqCounter.hasOwnProperty(inputArray[i])){
			freqCounter[inputArray[i]] = 0;
		}
		freqCounter[inputArray[i]]++;
	}
	return freqCounter;
}
